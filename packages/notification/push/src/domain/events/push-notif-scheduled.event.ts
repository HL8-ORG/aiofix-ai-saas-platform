...
/**
 * @class PushNotifScheduledEvent
 * @description
 * 推送通知调度事件，表示推送通知已成功调度到指定时间发送。
 *
 * 事件含义：
 * 1. 表示推送通知已成功调度到指定时间发送
 * 2. 包含推送通知调度时的关键信息
 * 3. 为其他聚合根提供推送通知调度通知
 * 4. 触发推送通知的调度监控和跟踪
 *
 ...
 * @property {Date} scheduledAt 计划发送时间
 * @property {Date} occurredOn 事件发生时间
 *
 * @example
 * ```typescript
 * const event = new PushNotifScheduledEvent(
 *   'push-123',
 *   'tenant-456',
 *   'user-789',
 *   pushToken,
 ...
 * @since 1.0.0
 */
export class PushNotifScheduledEvent implements IDomainEvent {
   ...
   */
  toString(): string {
    return `PushNotifScheduledEvent: ${this.pushNotifId} for user ${this.userId} in tenant ${this.tenantId} scheduled at ${this.scheduledAt}`;
  }
}

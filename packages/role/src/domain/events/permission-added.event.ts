 ...
 * @property {Permission} permission 添加的权限
 * @property {TenantId} tenantId 所属租户ID
 * @property {Date} addedAt 权限添加时间
 *
 * @example
 * ```typescript
 ...
 * @since 1.0.0
 */
export class PermissionAddedEvent extends DomainEvent {
  constructor(
   ...
   * @returns {PermissionAddedEvent} 事件实例
   * @static
   */
  static fromJSON(data: Record<string, unknown>): PermissionAddedEvent {
    return new PermissionAddedEvent(
      ...
      new Date(data.addedAt as string),
    );
  }

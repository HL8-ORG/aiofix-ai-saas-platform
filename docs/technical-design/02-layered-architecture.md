...
3. **事件产生**：领域层聚合产生领域事件
4. **事件发布**：应用层事件总线发布事件
5. **事件存储**：基础设施层将事件持久化到事件存储
6. **消息队列**：事件发布到消息队列进行异步处理
7. **事件处理**：异步事件处理器处理事件
8. **读模型更新**：事件处理器更新读模型
9. **查询响应**：查询处理器从读模型返回数据

### 数据一致性保证

- **最终一致性**：通过事件驱动架构实现读模型的最终一致性
- **事务边界**：命令处理在事务边界内，确保事件存储的一致性
- **消息队列可靠性**：消息队列提供持久化和重试机制
- **事件处理器重试**：事件处理器支持重试，确保读模型最终更新成功
- **死信队列处理**：失败的事件进入死信队列，支持手动处理
